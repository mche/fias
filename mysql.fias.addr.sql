...
CREATE DEFINER=`devel`@`%` PROCEDURE `addr`(regcode INT, word VARCHAR(255))
(
SELECT DISTINCT
				l4.l7AOID AS aoid,
...
FROM (SELECT
					...
					LOCATE(UPPER(SHORTNAME), 'ГСК СНТ САД') AS PRIOR
				FROM
					AS_ADDROBJ USE INDEX (REGLEVFN, FTFORMALNAME)
				WHERE
				(
					LIVESTATUS = 1
					AND AOLEVEL IN (3,4,5,6, 7)
					...
					#AND (FORMALNAME LIKE CONCAT(word, '%')) --  or FORMALNAME LIKE 'шлюз%'
					AND ( 
						MATCH(FORMALNAME) AGAINST(CONCAT(word, '*') IN BOOLEAN MODE)
						# OR
						# MATCH(FORMALNAME) AGAINST('+ким' IN BOOLEAN MODE)
					)
					#AND (FORMALNAME LIKE CONCAT('%', word, '%'))
					#AND match(OFFNAME) against ('+гск*' in boolean mode)
					AND REGIONCODE = regcode -- AND(a9.AOGUID<>@ParentGUID)
...
#ORDER BY l5CENTSTATUS DESC, l5AOLEVEL, l6CENTSTATUS DESC, l6AOLEVEL DESC, l7IFNSUL, l7FORMALNAME, l6FORMALNAME$$
ORDER BY PRIOR, l6CENTSTATUS DESC, l6AOLEVEL, l5CENTSTATUS DESC, l5AOLEVEL, l6FORMALNAME
LIMIT 500$$
